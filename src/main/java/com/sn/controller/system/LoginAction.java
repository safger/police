	...
	public String login(String username,String password){
		if(username!=null&&password!=null){
			...
			if(baseUser_list!=null&&baseUser_list.size()>0){
				...
				request.getSession().setAttribute("functional",organize.getFax());
				//---------------- 查询是否要提醒
				if(organize.getFax()==null||!organize.getFax().equals("1")){
					Problemdata problemdata=new Problemdata();
					problemdata.setCdepartmentid(departmentid);
					Integer a=problemdataService.countByColumDay(problemdata);
					if(a==null||a==0){
						request.getSession().setAttribute("jg","jg");
					}
				} 
